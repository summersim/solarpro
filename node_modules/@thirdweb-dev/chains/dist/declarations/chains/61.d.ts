declare const _default: {
    readonly chain: "ETC";
    readonly chainId: 61;
    readonly explorers: readonly [{
        readonly name: "blockscout";
        readonly url: "https://blockscout.com/etc/mainnet";
        readonly standard: "EIP3091";
    }];
    readonly faucets: readonly [];
    readonly features: readonly [{
        readonly name: "EIP155";
    }];
    readonly infoURL: "https://ethereumclassic.org";
    readonly name: "Ethereum Classic Mainnet";
    readonly nativeCurrency: {
        readonly name: "Ethereum Classic Ether";
        readonly symbol: "ETC";
        readonly decimals: 18;
    };
    readonly networkId: 1;
    readonly rpc: readonly ["https://ethereum-classic.rpc.thirdweb.com/${THIRDWEB_API_KEY}", "https://61.rpc.thirdweb.com/${THIRDWEB_API_KEY}", "https://etc.rivet.link", "https://etc.etcdesktop.com", "https://etc.mytokenpocket.vip"];
    readonly shortName: "etc";
    readonly slip44: 61;
    readonly slug: "ethereum-classic";
    readonly testnet: false;
};
export default _default;
//# sourceMappingURL=61.d.ts.map